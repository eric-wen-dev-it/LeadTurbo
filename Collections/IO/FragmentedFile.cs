...
using LeadTurbo.Interfaces;
using LeadTurbo.Synchronization;

namespace LeadTurbo.IO
{
    /// <summary>
    ...
    public class FragmentedFile : IDisposable
    {
        ...
        private Stream OpenBlock(FileAccess access, long identity)
        {
            ...
            try
            {
                ...
                long lastAllocated = LastAllocated(stream);
                if (offset < 0 || offset > lastAllocated)
                    throw new ArgumentOutOfRangeException(nameof(identity),
                        $"Block offset {offset} is outside valid range [0, {lastAllocated}].");
        ...
        class FileBlock : IDisposable
        {
            ...
            public FileBlock(int blockSize, bool alignIo) 
            {
                ...
                if (_alignedIo)
                {
                    ...
                    // blockSize is validated to be in [512, 65536], so (blockSize - 1) fits in int.
                    // The mask extracts the alignment offset within one block, always in [0, blockSize-1].
            ...
            // Finalizer ensures the GCHandle is released if Dispose() is not called,
            // preventing the pinned buffer from blocking garbage collection indefinitely.
            ~FileBlock()
            {
