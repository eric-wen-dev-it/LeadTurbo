...
namespace LeadTurbo.Collections
{
        ...
        IDictionaryEx<TKey, TValue>, IConcurrentDictionary<TKey, TValue>
    {
        ...
        /// <see cref="T:LeadTurbo.Collections.ICreateOrUpdateValue`2"/> interface.
        /// </summary>
        ...
        /// <see cref="T:LeadTurbo.Collections.IRemoveValue`2"/> interface.
        /// </summary>
...
#pragma warning restore 612,618
        ...
        void InternalLink(int index)
        {
            ...
            // 等待 next 节点的 Prev 被前驱线程清零后，再写入 index
            var spinner1 = new SpinWait();
        ...
        void InternalUnlink(int index)
        {
            while (true)
            {
                ...
                // 原子标记自身 Prev（置为 ~prev），CAS 失败时用最新值重试
                int prev = _entries[index >> _shift][index & _shiftMask].Prev;
                var spinnerPrev = new SpinWait();
                while (prev >= 0)
                {
                    cmp = Interlocked.CompareExchange(
                        ref _entries[index >> _shift][index & _shiftMask].Prev, ~prev, prev);
                    if (cmp == prev) break;
                    prev = cmp;
                    spinnerPrev.SpinOnce();
                }
                if (prev < 0)
                    throw new LurchTableCorruptionException();

                // 原子标记自身 Next（置为 ~next），CAS 失败时用最新值重试
                int next = _entries[index >> _shift][index & _shiftMask].Next;
                var spinnerNext = new SpinWait();
                while (next >= 0)
                {
                    cmp = Interlocked.CompareExchange(
