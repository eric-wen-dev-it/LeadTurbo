...
namespace LeadTurbo.Collections
{
        ...
        class Node
        {
            ...
            public static Node FromElements(IStorageHandle handle, bool isRoot, int nodeSize, Element[] items)
            {
                if(isRoot)
                {
                    ...
                    if (items.Length != 1)
                        throw new ArgumentException("Root node must contain exactly one element.", nameof(items));
                ...
                if (items.Length > nodeSize)
                    throw new ArgumentException($"items count ({items.Length}) exceeds node capacity ({nodeSize}).", nameof(items));
            ...
            {
                if (_ltype == LockType.Read)
                    throw new InvalidOperationException("Node is already read-only.");
                _ltype = LockType.Read;
                return this;
            }
            ...
            public virtual Node CloneForWrite(LockType ltype)
            {
                ...
                if (ltype == LockType.Read)
                    throw new InvalidOperationException("Read lock cannot clone for write.");
            ...
            {
                if (IsRoot)
                    throw new InvalidOperationException("Invalid operation on root.");
                if (_ltype == LockType.Read)
                    throw new InvalidOperationException("Node is currently read-only.");
                if (ordinal < 0 || ordinal >= _count)
                    throw new ArgumentOutOfRangeException(nameof(ordinal), "Index out of range.");
            ...
            public void ReplaceChild(int ordinal, NodeHandle original, NodeHandle value)
            {
                ...
                bool childMatches = (original == null && replacing.ChildNode == null) ||
                                    (original != null && original.Equals(replacing.ChildNode));
                if (!childMatches)
                    throw new ArgumentException("Incorrect child being replaced.", nameof(original));
            ...
            public void SetValue(int ordinal, TKey key, TValue value, IComparer<TKey> comparer)
            {
                ...
                if (ordinal < 0 || ordinal >= _count)
                    throw new ArgumentOutOfRangeException(nameof(ordinal), "Index out of range.");
                if (comparer.Compare(_list[ordinal].Key, key) != 0)
                    throw new ArgumentException("Incorrect key for value replacement.", nameof(key));
            ...
            public void Insert(int ordinal, Element item)
            {
                ...
                if (_count >= _list.Length)
                    throw new InvalidOperationException("Node is full; cannot insert.");
            ...
            public void Remove(int ordinal, Element item, IComparer<TKey> comparer)
            {
                ...
                if (ordinal < 0 || ordinal >= _count)
                    throw new ArgumentOutOfRangeException(nameof(ordinal), "Index out of range.");
                if (comparer.Compare(_list[ordinal].Key, item.Key) != 0)
                    throw new ArgumentException("Key mismatch: element at ordinal does not match item being removed.", nameof(item));
