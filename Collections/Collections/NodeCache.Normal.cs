...
namespace LeadTurbo.Collections
{
    partial class BPlusTree<TKey, TValue>
    {
        ...
        sealed class NodeCacheNormal : NodeCacheBase
        {
                ...
                ~CacheEntry()
                {
                    ...
                    if (!_owner._disposed)
                    {
                        try
                        {
                            ...
                            {
                                System.WeakReference<CacheEntry> me;
                                if (_owner._cache.TryGetValue(Handle, out me) && !me.TryGetTarget(out _))
            ...
            private readonly Dictionary<NodeHandle, System.WeakReference<CacheEntry>> _cache;
            ...
            public NodeCacheNormal(BPlusTreeOptions<TKey, TValue> options) : base(options)
            {
                ...
                _cache = new Dictionary<NodeHandle, System.WeakReference<CacheEntry>>();
            ...
            {
                System.WeakReference<CacheEntry> weakRef;
                        ...
                        if (_cache.TryGetValue(handle, out weakRef))
                        {
                            if (!weakRef.TryGetTarget(out entry))
                                using (new SafeLock<DeadlockException>(weakRef))
                                {
                                    if (!weakRef.TryGetTarget(out entry))
                                    {
                                        entry = new CacheEntry(this, handle);
                ...
                if (entry == null)
                {
                    using (_cacheLock.Write(base.Options.LockTimeout))
                    {
                        if (!_cache.TryGetValue(handle, out weakRef))
                        {
                            _cache.Add(handle, weakRef = new System.WeakReference<CacheEntry>(entry = new CacheEntry(this, handle)));
                            handle.SetCacheEntry(weakRef);
                        }
                        else
                        {
