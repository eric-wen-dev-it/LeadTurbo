...
namespace LeadTurbo.Collections
{
    partial class BPlusTree<TKey, TValue>
    {
        ...
        struct Element
        {
            ...
            // IsValue is true for leaf values including null-valued elements (IsEmpty handles uninitialized slots)
            public bool IsValue { get { return !(_child is NodeHandle); } }
            public bool IsEmpty { get { return ReferenceEquals(null, _child); } }

            public NodeHandle ChildNode
            {
                get
                {
                    if (!IsNode)
                        throw new InvalidOperationException("Element does not contain a child node.");
                    return (NodeHandle)_child;
                }
            }
            ...
            public TValue Payload
            {
                ...
                {
                    if (IsNode)
                        throw new InvalidOperationException("Element contains a child node, not a value.");
                    return (TValue)_child;
                }
            }
